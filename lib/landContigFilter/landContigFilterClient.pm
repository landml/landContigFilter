package landContigFilter::landContigFilterClient;
...
landContigFilter::landContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => landContigFilter::landContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a landContigFilter.FilterContigsParams
$output is a landContigFilter.FilterContigsResults
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a landContigFilter.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "landContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
=head2 filter_contigs_max
  ...
  $output = $obj->filter_contigs_max($params)
...
<pre>
$params is a landContigFilter.FilterContigsMaxParams
$output is a landContigFilter.FilterContigsResults
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "landContigFilter.filter_contigs_max",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "landContigFilter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "landContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for landContigFilter::landContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "landContigFilter::landContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package landContigFilter::landContigFilterClient::RpcClient;
